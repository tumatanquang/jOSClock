...
Java_uc_j_OSClock_nanoTime(JNIEnv *env, jclass cls) {
	...
	uint64_t ns = (uint64_t)((uint64_t)li.QuadPart * nanos_per_count);
	return (jlong)ns;
#elif defined __APPLE__
	uint64_t tm;
	...
	uint64_t ns = (uint64_t)(tm * mach_timebase_ratio);
	return (jlong)ns;
#elif defined CLOCK_MONOTONIC_COARSE
	struct timespec ts;
	clock_gettime(CLOCK_MONOTONIC_COARSE, &ts);
	uint64_t ns = (uint64_t)ts.tv_sec * 1000000000LL + (uint64_t)ts.tv_nsec;
	return (jlong)ns;
	...
	struct timeval tv;
	gettimeofday(&tv, NULL);
	uint64_t ns = (uint64_t)tv.tv_sec * 1000000000LL + (uint64_t)tv.tv_usec * 1000LL;
	return (jlong)ns;
#endif
}
